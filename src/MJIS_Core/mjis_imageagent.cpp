...
namespace mjis {
    ...
    ImageAgent::ImageAgent(hmi::Tabwidget* tab) :tab(tab), Camera(nullptr), save(nullptr), subtitle(nullptr), titlestate(false)
    {
    ...
    */
    ...
    //根据返回的ID创建窗口
    void ImageAgent::CreateCamera(uint idex, camera::ICamera::CameraType Type)
    {
        if (Camera == nullptr) {
            switch (Type)
            {
            case camera::ICamera::No:
                return;
            case camera::ICamera::Local_Camera:
                Camera = std::shared_ptr<camera::ICamera>(new camera::LocalCamera());
                break;
            case camera::ICamera::MV_Camera:
                //Camera = std::shared_ptr<camera::ICamera>(new camera::MVCamera());
                break;
            default:
                return;
            }
        ...
        if (tab != nullptr) {
            ...
            connect(Camera.get(), &camera::ICamera::cameraReadQImage, this, &ImageAgent::ReadQImage);//将读取到的数据送入数据处理线程
            connect(this, &ImageAgent::UpdateImage, tab, &hmi::Tabwidget::ShowCameraQImage);//将处理后的图像写入UI
        }
    }


    //返回相机指针
    camera::ICamera* ImageAgent::ReturnCamera()
    {
        return Camera.get();
    }
    ...
    void ImageAgent::OpenImage(const QString& path)
    {
        ...
        connect(this, &ImageAgent::UpdateImage, tab, &hmi::Tabwidget::ShowCameraQImage);
