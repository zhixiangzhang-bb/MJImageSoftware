...
namespace mjis {
	...
	//创建图像处理及相机类
	void Presenter::CreateImageAgent(hmi::Tabwidget* tab)
	{
	...
	//创建相机实例
	void Presenter::CreateCamera(uint idex, camera::ICamera::CameraType Type, int page)
	{
	...
	//返回相机实例指针
	camera::ICamera* Presenter::ReturnCameraPoint(int page)
	{
