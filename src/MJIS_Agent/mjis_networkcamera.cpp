...
{
    std::shared_ptr<mjis::ICamera> Cam(new mjis::MVCamera);
    int num=Cam->search();
    if(num<=0){
        ...
        for (int var = 0; var < num; ++var) {
            ...
            Cam->setidx(var);
            mjis::ICamera::Message ms=Cam->gainmessage();
...
void networkcamera::on_apply_clicked()
{
    ...
    if(!items.empty()){
        ...
        case 0://项0返回MV_EN系列相机
            emit SelcetNetcamId(ui->tableWidget->row(items.at(0)), mjis::ICamera::MV_Camera);
            this->close();
            break;
        case 1:
            break;
