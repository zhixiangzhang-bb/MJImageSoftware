...
//添加连接
void DataInterface::AddTCPConnect(const QString& name, const QString& IP, std::shared_ptr<NetConnect> tcp)
{
...
//创建连接
void DataInterface::CreateConnect(const QString& name, const QString& sshIP, int sshPort, NetConnect::ConnectStyle Style, const QString& localIP, int localPort)
{
    std::shared_ptr<NetConnect> cnct;
    switch (Style)
    {
    case NetConnect::TCPCilent:
        cnct=std::make_shared<ZTCPCilent>();
        break;
    case NetConnect::TCPServer:
        cnct = std::make_shared<ZTCPServer>();
        break;
    case NetConnect::UDPCilent:
        break;
    case NetConnect::UDPServer:
        break;
    case NetConnect::ModbusTCPServer:
        break;
    case NetConnect::ModbusTCPCilent:
        break;
    default:
        break;
    }
    ...
    connect(cnct.get(), &NetConnect::ReadData, dcm.get(), &DataConvertManage::ConverReadData);
    connect(dcm.get(), &DataConvertManage::ReadData, this, &DataInterface::NetWorkData);
    connect(cnct.get(), &NetConnect::ConnectError, this, &DataInterface::ConnectError);
    connect(cnct.get(), &NetConnect::ConfigChange, this, &DataInterface::ConnectConfigChange);
...
//新建返回的TCP参数进行设置
void DataInterface::ReturnTCPConfig(const QString& name, const QString& sshIP, int sshPort, NetConnect::ConnectStyle Style, const QString& localIP, int localPort)
{
...
{
    NetConnect* netsender = qobject_cast<NetConnect*>(sender());
    if (netsender) {
...
void DataInterface::ReadConnectConfig()
{
    ...
    if (std::equal(numbers.begin(), numbers.end(), numbers.begin())) {
        ...
        {
            CreateConnect(name[i], sshIP[i], sshPort[i], static_cast<NetConnect::ConnectStyle>(style[i]), localIP[i], localPort[i]);
        }
    }
...
void DataInterface::ConnectConfigChange()
{
    ...
    QMap<std::shared_ptr<NetConnect>, std::shared_ptr<DataConvertManage>>::const_iterator iter;
